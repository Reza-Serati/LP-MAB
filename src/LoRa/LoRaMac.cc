...
{
    ////////////////////////////////////////////
//    cancelAndDelete(replicatePacket);
    ////////////////////////////////////////////
...
//    std::cout<<"LoRaMac:: adrMethod: " << adrMethod << endl;
    /////////////////////////////////////////////////////////
    MacProtocolBase::initialize(stage);
    if (stage == INITSTAGE_LOCAL) {
        EV << "Initializing stage\n";
...
//            kn.srcAddr = address;
//            char buffer[50];
//            strcpy(buffer, "Replicate_");
//            strcat(buffer, address.str().c_str());
//            kn.replicatePacket = new cMessage(buffer);
//            kn.max_interval = couasd++;
////            kn.fsm.
//            macKnownNodes.push_back(kn);
////            macKnownNodes.insert(kn);
//            for(auto & elem : macKnownNodes){
////                if(elem.srcAddr == address){
//                    std::cout<< macKnownNodes.size() << elem.srcAddr << std::endl;
////                }
//            }
...
//        }
        ...
        /////////////////////////////////////////////////
        replicatePacket = new cMessage("Replicate");
        /////////////////////////////////////////////////
...
//    LoRaMacControlInfo *cInfo = check_and_cast<LoRaMacControlInfo *>(msg->getControlInfo());
    ...
    auto &frame = pktEncap->peekAtFront<LoRaMacFrame>();
...
//            auto pkt = check_and_cast<Packet *>(msg);
//                const auto & packet = pkt->peekAtFront<LoRaAppPacket>();
//            std::cout<<"packet->getOptions().getLoRaTP: " << packet->getOptions().getLoRaTP() << endl;
...
//            Ptr<LoRaMacFrame> frame = dynamicPtrCast<LoRaMacFrame>(constPtrCast<Chunk>(chunk));
//            std::cout<<"frame.get()->getLoRaSF: " << frame.get()->getLoRaSF() << endl;
//        }
...
void LoRaMac::handleWithFsm(cMessage *msg)
{
    ...
    FSMA_Switch(fsm)
    {
            ...
            if(adrMethod == "serati"){
                if(max_interval == 1){
                    FSMA_Event_Transition(Idle-Transmit,
                            ...
                            TRANSMIT,
                            if(printttt){
                                std::cout<<"MAC Sending msg with seq# " << sequenceNumber << std::endl;
                                std::cout<<"\tTry #" << interval_count << std::endl;
                            }
                    );
                }
                else{
                    FSMA_Event_Transition(Idle-Replicate,
                            isUpperMessage(msg),
                            REPLICATE,
                            interval_count = 1;
                            retransmiting = true;
        ...
        FSMA_State(REPLICATE){
            if(interval_count == max_interval){
                FSMA_Event_Transition(Replicate-Transmit,
                    true,
                    TRANSMIT,
                    );
            }
                ...
                FSMA_Event_Transition(Replicate-Replicate,
                    true,
                    REPLICATE,
                    if(printttt){
                        std::cout<<"\tTry #" << interval_count << std::endl;
                ...
                cancelEvent(replicatePacket);
                scheduleAt(simTime() + sending_interval_time, replicatePacket);
                numSent++;
                retransmiting = false;
            }
        }
...
Packet *LoRaMac::encapsulate(Packet *msg)
{
    ...
    ////////////////////////////////
    auto tag = msg->getTagForUpdate<LoRaTag>(); // changed from getTag to getTagForUpdate
    /////////////////////////////////////
    if(adrMethod == "serati"){
        int tmpSF = tag->getSpreadFactor();
        if(tmpSF > 12){
            Customize_Table c_table;
            int* tmp = c_table.split_bit_str(c_table.int_to_bit_string(tmpSF, 16), 8);
            tag->setSpreadFactor(tmp[0]);
            max_interval = tmp[1];
        }
    }
...
//    std::cout<<"End Node Sending " << simTime() << "\n";
